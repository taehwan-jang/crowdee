...
public class CustomOAuth2MemberService extends DefaultOAuth2UserService {
    ...
    private static final String INVALID_USER_NAME_INFO_RESPONSE_ERROR_CODE = "invalid_user_info_response";
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        if(!StringUtils.hasText(userNameAttributeName)){
            OAuth2Error oAuth2Error = new OAuth2Error(
                    ...
                    "Missing required \"user name\" attribute name in UserInfoEndPoint for " +
                            "Client Registration: " + userRequest.getClientRegistration().getRegistrationId(), null);
            throw new OAuth2AuthenticationException(oAuth2Error, oAuth2Error.toString());
        }
        ...
        try{
            ...
            errorDetails.append("UserInfo Uri: ").append(
                    ...
                    errorDetails.append("]");
                    oauth2Error = new OAuth2Error(INVALID_USER_NAME_INFO_RESPONSE_ERROR_CODE,
                            "An error occurred while attempting to retrieve the UserInfo Resource: "
                    + errorDetails.toString(), null);
                    throw new OAuth2AuthenticationException(oauth2Error, oauth2Error.toString(), ex);
        }
        catch(RestClientException ex){
            OAuth2Error oAuth2Error = new OAuth2Error(INVALID_USER_NAME_INFO_RESPONSE_ERROR_CODE,
                    "An error occurred while attempting to retrieve the UserInfo Resource: "
                    + ex.getMessage(), null);
            throw new OAuth2AuthenticationException(oAuth2Error, oAuth2Error.toString(), ex);
        }
    ...
    private Map<String, Object> getUserAttributes(ResponseEntity<Map<String, Object>> response) {
        ...
        if(userAttributes.containsKey("response")){
            LinkedHashSet responseData = (LinkedHashSet) userAttributes.get("response");
            userAttributes.putAll((Map<? extends String, ?>) responseData);
